    ...
    with trips_container:
        if len(available_trips_df) == 0:
            ...
            if facility_filter and len(facility_filter) < len(all_facilities):
                ...
                st.info(f"Filtered to {len(available_trips_df)} trips from {', '.join(facility_filter)}")
            ...
            # Always show consistent columns - unified format
            display_columns = ['Select', 'from', 'trip_ref_number', 'hub', 'delivery_date', 'total_qty', 'total_value', 'product_count']
